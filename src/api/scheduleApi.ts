...
import type { StaffMember, Unavailability, WeeklyNeeds, ApiResponse, Schedule } from '../types'; // Ensure ApiResponse and Schedule are imported
...
export const generateScheduleAPI = async (requestData: RequestData): Promise<ApiResponse> => {
  ...
  try {
    ...
    try {
        ...
        if (text) {
            ...
            if(response.ok) {
                return { success: true, schedule: {}, warnings: ["Warning: Received empty success response from server."], calculationTimeMs: 0 };
            }
        }
    } catch (jsonError) {
    ...
    if (!response.ok) {
      ...
      const errorMessage = responseData?.message || `HTTP error ${response.status}${parseError ? ' (Failed to parse error details)' : ''}`;
    ...
    const apiResponse: ApiResponse = {
        ...
        calculationTimeMs: typeof responseData?.calculationTimeMs === 'number' ? responseData.calculationTimeMs : undefined,
