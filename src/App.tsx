...
import type { StaffMember, Unavailability, WeeklyNeeds, Schedule } from './types';
...
function App() {
  ...
  const [unavailabilityList, setUnavailabilityList] = useState<Unavailability[]>([]);
  ...
  const handleDeleteStaff = (idToDelete: string) => {
     if (confirm('Are you sure you want to delete this staff member and their unavailability?')) {
        const staffToDelete = staffList.find(s => s.id === idToDelete);
        setStaffList(prevList => prevList.filter(staff => staff.id !== idToDelete));
        setUnavailabilityList(prevUnav => prevUnav.filter(unav => unav.employeeId !== idToDelete));
  ...
  const handleAddUnavailability = (newUnavData: Unavailability) => {
   ...
   const handleDeleteUnavailability = (employeeId: string, dayOfWeek: string) => {
     if (confirm(`Are you sure you want to delete all unavailability for ${employeeId} on ${dayOfWeek}?`)) {
         setUnavailabilityList(prevList =>
             prevList.filter(item => !(item.employeeId === employeeId && item.dayOfWeek === dayOfWeek))
         );
         console.log(`Unavailability deleted for ${employeeId} on ${dayOfWeek}`);
     }
   };

   const handleNeedsChange = useCallback((day: string, shiftKey: string, role: string, count: number) => {
      ...
      try {
        ...
        if (responseData.success) {
          ...
          if (responseData.warnings && responseData.warnings.length > 0) {
              console.warn("Schedule generated with warnings:", responseData.warnings);
              alert("The schedule has been generated, but there are the following warnings:\n" + responseData.warnings.join("\n"));
          } else {
              alert("The schedule has been generated！");
          }
        } else {
          setSchedule(null); 
          const errorMessages = responseData.warnings || [responseData.message || "Unknown backend Error"];
    ...
    const handleImportStaff = useCallback((importedData: any) => {
      ...
      const isValid = importedData.every(item =>
          item && typeof item === 'object' && 'id' in item && 'name' in item && 'roles' in item && Array.isArray(item.roles)
      );
      if (!isValid) {
          alert("Import failed: Staff data structure is invalid. Required fields: id, name, roles (array).");
          return;
      }
       const importedIds = new Set();
       for (const item of importedData) {
           if (importedIds.has(item.id)) {
               alert(`Import failed: Duplicate staff ID found in imported data: ${item.id}.`);
               return;
           }
           importedIds.add(item.id);
       }
  
      if (confirm(`Import ${importedData.length} staff records? This will replace the current list and clear unavailability.`)) {
    ...
    const handleImportUnavailability = useCallback((importedData: any) => {
        if (!Array.isArray(importedData)) { alert("Import failed: Unavailability data must be an array."); return; }
        ...
        for (const item of importedData) {
              if (item && typeof item === 'object' && item.employeeId && typeof item.employeeId === 'string' &&
                  DAYS_OF_WEEK.includes(item.dayOfWeek) && Array.isArray(item.shifts) &&
                  item.shifts.every((s: any) => s && typeof s.start === 'string' && typeof s.end === 'string')
              ) {
        ...
        if(validatedData.length === 0 && importedData.length > 0) {
            alert("Import failed: No valid unavailability records found or all records belong to unknown staff.");
            return;
        }
  
        let message = `Import ${validatedData.length} valid unavailability records? This will replace the current list.`;
        if (invalidEntries > 0) message += ` (${invalidEntries} invalid/unknown records ignored)`;
    ...
    const handleImportNeeds = useCallback((importedData: any) => {
        if (typeof importedData !== 'object' || importedData === null || Array.isArray(importedData)) {
            alert("Import failed: Weekly needs data must be an object."); return;
        }
  
        let isValid = true; let errorMsg = "";
        for(const day in importedData) {
            if (!DAYS_OF_WEEK.includes(day)) { isValid = false; errorMsg=`Invalid day: ${day}`; break; }
            if (typeof importedData[day] !== 'object') { isValid = false; errorMsg=`Data for ${day} is not an object`; break; }
            for (const shiftKey in importedData[day]) {
                 if (!SHIFT_KEYS.includes(shiftKey)) { isValid = false; errorMsg=`Invalid shift key for ${day}: ${shiftKey}`; break; }
                 if (typeof importedData[day][shiftKey] !== 'object') { isValid = false; errorMsg=`Data for ${day}/${shiftKey} is not an object`; break; }
                 for (const role in importedData[day][shiftKey]) {
                     if (!ALL_ROLES.includes(role)) { isValid = false; errorMsg=`Invalid role for ${day}/${shiftKey}: ${role}`; break; }
                     const count = importedData[day][shiftKey][role];
                     if (typeof count !== 'number' || !Number.isInteger(count) || count < 0) {
                         isValid = false; errorMsg=`Invalid count for ${day}/${shiftKey}/${role}: ${count}`; break;
                     }
                 }
            }
            if (!isValid) break;
        ...
        if (!isValid) { alert(`Import failed: Invalid weekly needs structure. ${errorMsg}`); return; }
  
        if (confirm(`Import weekly needs data? This will replace the current settings.`)) {
            setWeeklyNeeds(importedData as WeeklyNeeds);
            console.log("Weekly needs data imported. Grid will reflect on next interaction or re-render.");
    ...
    <div className="container mx-auto p-4 lg:p-8 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="p-4 bg-white rounded-lg shadow">
             <h2 className="text-xl font-semibold mb-4 text-gray-700">Staff Management</h2>
             {/* --- Render Staff Components --- */}
             <StaffForm onAddStaff={handleAddStaff} />
             <StaffList staffList={staffList} onDeleteStaff={handleDeleteStaff} />
             {/* --- End Render Staff Components --- */}
          </div>
          {/* Unavailability Section */}
          <div className="p-4 bg-white rounded-lg shadow">
             <h2 className="text-xl font-semibold mb-4 text-gray-700">Staff Unavailability</h2>
             <ImportExportButtons
             ...
             <UnavailabilityForm staffList={staffList} onAddUnavailability={handleAddUnavailability} />
             <UnavailabilityList
                 unavailabilityList={unavailabilityList}
                 staffList={staffList}
        ...
        <div className="lg:col-span-2 space-y-6">
           <div className="p-4 bg-white rounded-lg shadow">
              <h2 className="text-xl font-semibold mb-4 text-gray-700">Needs Section</h2>
              <ImportExportButtons
                 dataType="Weekly Needs"
                 dataToExport={weeklyNeeds}
                 onDataImport={handleImportNeeds}
              />
              <NeedsInputGrid weeklyNeeds={weeklyNeeds} onNeedsChange={handleNeedsChange} />
           </div>
        </div>
      </div>
      ...
      <div className="mt-8 space-y-6">
         <div className="p-4 bg-white rounded-lg shadow text-center">
              <button  onClick={handleGenerateSchedule} disabled={isLoading} className={`px-6 py-3 text-lg font-semibold rounded-md text-white ${
                      isLoading ? 'bg-gray-400 cursor-not-allowed animate-pulse' : 'bg-green-600 hover:bg-green-700'
         ...
         <div className="p-4 bg-white rounded-lg shadow min-h-[200px]">
              <h2 className="text-xl font-semibold mb-4 text-gray-700">Generate Schedule Result</h2>
              {isLoading && <p className='text-center text-blue-600 animate-pulse'>Waiting the result from Web Server...</p>}
              {!isLoading && (
                  <ScheduleDisplay schedule={schedule} staffList={staffList} />
              )}
         </div>
         <div className="p-4 bg-white rounded-lg shadow">
             <h2 className="text-xl font-semibold mb-4 text-gray-700">Logs and Warnings</h2>
              {warnings.length > 0 && (
