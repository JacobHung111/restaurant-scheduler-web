...
function UnavailabilityList({ unavailabilityList, staffList, onDeleteUnavailability }: UnavailabilityListProps) {
    ...
    if (!groupedUnavailability[key]) {
     ...
     if (Array.isArray(item.shifts)) {
         item.shifts.forEach(newShift => {
             if (!groupedUnavailability[key].shifts.some(existing => existing.start === newShift.start && existing.end === newShift.end)) {
  ...
  const sortedGroupedItems = Object.values(groupedUnavailability).sort((a, b) => {
  ...
  return (
    <div className="mt-6">
      <h3 className="text-lg font-semibold mb-2 text-gray-700">Unavailability List:</h3>
        ...
        <ul className="space-y-2">
          {sortedGroupedItems.map((item) => {
             const shiftTexts = item.shifts
               ...
               .map(s => s.start === "00:00" && s.end === "23:59" ? "All Day" : `${s.start}-${s.end}`)
               .join(', ');
             const staffName = getStaffName(item.employeeId);
             return (
                <li key={`${item.employeeId}_${item.dayOfWeek}`} className="flex items-center justify-between p-3 bg-gray-100 rounded border border-gray-200">
                  <span className="text-sm text-gray-800">
                    <strong className="font-medium">{staffName}</strong> - {item.dayOfWeek}
                    <br />
                    <span className="text-xs text-gray-600">Unavailable: {shiftTexts}</span>
                  </span>
                  <button
                    onClick={() => onDeleteUnavailability(item.employeeId, item.dayOfWeek)}
