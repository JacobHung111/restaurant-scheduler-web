...
function processScheduleForTable(schedule: Schedule | null, staffList: StaffMember[]): ProcessedSchedule {
    ...
    for (const day of DAYS_OF_WEEK) {
        ...
        const assignmentsByStaff: { [empId: string]: { start: number, end: number, role: string }[] } = {};
            ...
            for (const role in schedule[day][shiftKey]) {
                ...
                if (Array.isArray(employeeIds)) {
                    for (const empId of employeeIds) {
                        ...
                        assignmentsByStaff[empId].push({ start: shiftStartMin, end: shiftEndMin, role });
                    }
                }
            }
        }
        for (const empId in assignmentsByStaff) {
            ...
            let currentShift: { start: number, end: number, role: string } | null = null;
            for (const assignment of assignments) {
                if (currentShift === null) { currentShift = { ...assignment }; }
                else if (assignment.start === currentShift.end && assignment.role === currentShift.role) { currentShift.end = assignment.end; }
                else {
                    const startStr = minutesToTime(currentShift.start); const endStr = minutesToTime(currentShift.end);
                    shiftsForTable.push({ shiftText: `${startStr}-${endStr}`, role: currentShift.role });
...
function ScheduleDisplay({ schedule, staffList }: ScheduleDisplayProps) {
  ...
  if (!schedule || Object.keys(schedule).length === 0) {
     const message = schedule === null ? "No schedule generated yet..." : "Schedule generated, but no shifts assigned...";
      ...
      <table className="min-w-full divide-y divide-gray-200 border border-gray-300">
        <thead className="bg-gray-100">
          <tr>
            ...
            <th scope="col" className="px-4 py-2 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider border-r border-gray-300 sticky left-0 bg-gray-100 z-10">
              Staff / Day
            </th>
            {/* Render Days of Week as column headers */}
            {DAYS_OF_WEEK.map(day => (
              <th key={day} scope="col" className="px-4 py-2 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap border-l border-gray-200">
                {day}
              </th>
            ))}
          </tr>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          ...
          {sortedStaff.map((staff, staffIndex) => (
            ...
            <tr key={staff.id} className={staffIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
              {/* Sticky cell for Staff Name */}
              <td className="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-800 border-r border-gray-300 sticky left-0 z-10"
                  style={{ backgroundColor: staffIndex % 2 === 0 ? 'white' : '#F9FAFB' }}>
                {staff.name}
              </td>
              ...
              {DAYS_OF_WEEK.map(day => {
                ...
                return (
                  <td key={`${staff.id}-${day}`} className="px-4 py-2 whitespace-nowrap text-xs text-gray-700 border-l border-gray-200 align-top">
                    {assignments && assignments.length > 0 ? (
                       // Display shifts, maybe stack vertically if multiple
                       <div className="flex flex-col space-y-1">
                           {assignments.map((a, index) => (
                               <span key={index} className="block">{`${a.shiftText}`}</span>
                           ))}
                       </div>
                    ) : (
